...
void move(int snakeXY[][SNAKE_ARRAY_SIZE], int snakeLength, int direction)
{
	...
	gotoxy1(snakeXY[0][0],snakeXY[1][0]);	
	printf("%c", SNAKE_BODY);
	...
	gotoxy1(1,1); //Gets rid of the darn flashing underscore.
...
void loadEnviroment(int consoleWidth, int consoleHeight){
	...
	gotoxy1(x,y); //Top left corner
	
	for (; y < rectangleHeight; y++)
	{
		gotoxy1(x, y); //Left Wall 
		printf("%c",WALL);
		
		gotoxy1(consoleWidth, y); //Right Wall
		printf("%c",WALL);
	}
	...
	for (; x < consoleWidth+1; x++)
	{
		...
		gotoxy1(x, rectangleHeight); //Right Wall
		printf("%c",WALL);
	}
	return;
}
...
void loadSnake(int snakeXY[][SNAKE_ARRAY_SIZE], int snakeLength)
{
	...
	{
		gotoxy1(snakeXY[0][i], snakeXY[1][i]);
		printf("%c", SNAKE_BODY);
	}
